...
public record InviteResponse(UUID id, String email, TargetType targetType, UUID targetId, String role,
		UUID invitedById, String invitedByName, InviteStatus status, Instant createdAt, Instant sentAt,
		Instant lastResentAt, Instant expiresAt, Instant acceptedAt, UUID claimedUserId, boolean isValid,
		boolean isExpired) {
	...
	public static InviteResponse from(Invite invite) {
		return new InviteResponse(invite.getId(), invite.getEmail(), invite.getTargetType(), invite.getTargetId(),
				...
				invite.getCreatedAt(), invite.getSentAt(), invite.getLastResentAt(), invite.getExpiresAt(),
